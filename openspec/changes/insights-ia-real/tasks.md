...
5. **Inconsistencia de checklist — tarea 1.15 marcada `[x]` pese a depender de 1.14 (sin marcar)**: el criterio de finalización de 1.15 exige completar el refactor `analyzeFinances`→`generateIdea`, `predictNextMonthExpenses`→`predict` y `mapCSVColumns`→`mapColumns`, además de preservar los exports de compatibilidad. En el código real (`src/lib/groqProvider.js:169-170`), `generateIdea` y `predict` son stubs que rechazan explícitamente con `not_implemented: ... fase posterior` — no están implementados. El `[x]` de 1.15 es más amplio que lo que efectivamente quedó construido: no representa la tarea completa tal como la describe su propio criterio de finalización. Esta es una **inconsistencia del checklist del ciclo original (2026-07-15)**, detectada en el diagnóstico de estado del 2026-08-07 — no se corrige retroactivamente el `[x]` ni se reinterpreta qué quedó hecho; se deja registrada como hallazgo.
