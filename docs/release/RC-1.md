...
Nota sobre "Media": RC-1.2, RC-1.3, RC-1.4, RC-1.5, RC-1.6 y RC-1.7 tienen su etapa de auditoría cerrada (ver detalle en cada ticket), pero ninguno cuenta como "Resuelto" en esta tabla porque los seis conservan alcance remanente (RC-1.2: D1 resuelta (description obligatorio) — C1 implementado, A2 implementado (orden alfabético de categorías), M1 confirmado sin acción, B1–B4 retirados por pérdida de trazabilidad documental — alcance implementable completo; RC-1.3: hallazgos M4/M5/M6 pendientes de decisión de diseño; RC-1.4: bloqueado únicamente por D2, decisión de producto pendiente, y solo para C2 — A3 fue clasificado como deuda de integración, no como decisión de producto, ver `docs/design/integration-debt.md`; M1–M5/B1–B6 retirados por pérdida de trazabilidad documental; alcance implementable restante: C1, A1, A2; RC-1.5: hallazgo funcional del roadmap (formatCurrency/exportUtils) implementado — alcance del roadmap completo; sin decisión de producto pendiente; la adopción de i18n (migración de textos por pantalla) queda diferida a una fase posterior al Release Candidate, fuera del roadmap de RC-1; RC-1.6: hallazgo A4/M3 clasificado como deuda de integración, no como decisión de producto, ver `docs/design/integration-debt.md`; C1 implementado; A1–A3/M1–M6/B1–B4 retirados por pérdida de trazabilidad documental (ver detalle en la sección del ticket); RC-1.7: 4 hallazgos con revisión editorial de clasificación aplicada — todo hallazgo con descripción recuperable ya fue implementado (C1, A1, A3, A4, A5, A6, M1, M2, M3, M4); M6 diferido a Fase 3/RC-1.6 por decisión del PO; B1–B3 sin contenido recuperable en este documento ni en memoria, cerrados sin acción). En los seis, la auditoría está cerrada pero la implementación todavía no fue realizada en su totalidad. RC-1.3/A2 y RC-1.7 (salvo M6/B1–B3, ver detalle) son los que más avanzaron en implementación efectiva.
...
Estado:
✅ Alcance de esta fase completo. **D1 resuelta** (el PO decidió: `description` debe ser obligatorio, no opcional) — **C1 implementado** (ver detalle abajo). **A2 implementado** (orden alfabético de categorías sugeridas, ver detalle abajo). **M1** confirmado, sin acción — ver detalle abajo. **B1–B4 retirados** por pérdida de trazabilidad documental — ver "Regularización documental" a continuación. M2/M3 (fecha/cuenta no editables en creación) siguen sin acción, ya documentados como deuda de Fase III — no son hallazgos accionables. M4 (integration-debt.md desactualizado tras RC-1.1) fuera del alcance de esta fase (no forma parte del roadmap de Fase 5), sin verificar.
...
**C1 — implementado y commiteado (`f8ae378`).** Con un importe válido y una descripción vacía o de menos de 3 caracteres, guardar fallaba en silencio — sin mensaje de error, sin toast, sin cierre de la hoja. Causa raíz: `NewMovementSheet.jsx` no validaba `description` del lado del cliente (solo `isAmountValid` bloqueaba el submit), y en `useTransactions.js` el `showAlert('error', ...)` de `addIncome`/`addExpense` estaba condicionado a `notification === 'legacy'` — con `notification: 'toast'` (el modo usado en producción desde el Checkpoint IV-C) el fallo de validación no mostraba nada. Se agrega validación client-side de `description` en `NewMovementSheet.jsx` reutilizando `validateDescription` de `src/utils/validators.js` (no una regla nueva), mismo patrón ya existente para `isAmountValid`: bloquea el submit y muestra el error inline si está vacía o tiene menos de 3 caracteres. En `useTransactions.js` se quita la condición `notification === 'legacy'` del `showAlert` de error en `addIncome`/`addExpense` — línea de defensa adicional para que ningún fallo de validación que llegue hasta el hook quede mudo. `updateIncome`/`updateExpense` no se tocan: confirmado que su `showAlert('error', ...)` ya es incondicional, sin el mismo gap. Evidencia: 3 tests nuevos (`NewMovementSheet.test.jsx`: Enter con descripción vacía, Enter con descripción de menos de 3 caracteres, corrección tras intento fallido habilita "Añadir" y guarda) + 2 tests preexistentes corregidos en el mismo archivo (requerían el paso de completar descripción, agregado por este cambio) + 2 tests preexistentes en `useTransactions.test.js` que asertaban el comportamiento silencioso como correcto, reescritos para asertar la corrección (`notification:'toast'` con datos inválidos ahora sí llama a `showAlert`), suite completa en verde (62 archivos / 708 tests), build en verde, y validación en navegador real (Nuevo movimiento → Gasto): importe válido + descripción vacía → botón "Añadir" deshabilitado y error inline "La descripción es requerida"; descripción de 2 caracteres → error inline "La descripción debe tener al menos 3 caracteres"; al corregir con una descripción válida el botón se habilita y el movimiento se guarda correctamente (confirmado contra el store simulado de Supabase), sin errores de consola.

**A2 — implementado y commiteado (`aff709f`).** `EXPENSE_CATEGORIES` (`src/constants/categories.js`) no seguía ningún criterio de orden explícito (Vivienda, Alimentación, Transporte, Entretenimiento, Salud, Educación, Servicios, Otros). El PO definió orden alfabético; antes de implementar se verificó (a) si existía alguna referencia documental sobre tratamiento especial de "Otros" — no se encontró ninguna, solo una mención no relacionada en `openspec/changes/insights-ia-real/spec.md` sobre exclusión de "Otros" del conteo de elegibilidad para insights de IA — y (b) todos los consumidores del array, confirmando que 6 `<select>`/listas de filtro dependen del orden de renderizado (esperado, es el propósito de A2) y que `BudgetManager.jsx` reordena sus chips de "otras categorías disponibles" en consecuencia (esperado); `CategoryChart.jsx` y la validación de `useTransactions.js` son independientes del orden (lookup/membership). Se detectó un efecto colateral fuera del alcance original de A2: `NewMovementSheet.jsx`, `RecurringManager.jsx` y `SharedSpaceManager.jsx` usan `EXPENSE_CATEGORIES[0].value` como categoría por defecto preseleccionada — el PO decidió aceptar el cambio de default (de "Vivienda" a "Alimentación") en vez de desacoplarlo con una constante separada, para no ampliar el alcance a 3 archivos adicionales. Decisión del PO sobre "Otros": queda **fija al final**, fuera del orden alfabético estricto (catch-all). Orden final: Alimentación, Educación, Entretenimiento, Salud, Servicios, Transporte, Vivienda, Otros. Cambio acotado exclusivamente a `src/constants/categories.js`. Evidencia: suite completa en verde (62 archivos / 708 tests, sin regresiones — los tests existentes derivan sus expectativas del propio array por índice, no de nombres hardcodeados), build en verde, y validación en navegador real (Nuevo movimiento → Gasto → Elegir categoría): el `<select>` muestra las 8 opciones en el nuevo orden alfabético con "Otros" al final, y el valor preseleccionado por defecto es "Alimentación", sin errores de consola.

**Con esto, el alcance implementable de RC-1.2 queda completo** (C1, A2, M1 confirmado sin acción, B1–B4 retirados). D1/D2 eran las únicas decisiones de producto pendientes de esta fase — D1 ya resuelta; D2 no forma parte del alcance de RC-1.2 (bloquea únicamente RC-1.4/C2).
...
Estado:
🟡 Auditoría cerrada — implementación pendiente. Hallazgo C2 (categorías del picker de revisión de importación inconsistentes con `EXPENSE_CATEGORIES`) reclasificado y sacado de alcance — ver "D2" en Decisiones de producto pendientes, arriba (única decisión de producto pendiente de este ticket). Hallazgo A3 (arquitectura de información de "Herramientas" vs. Product Blueprint) analizado y clasificado: **no constituye una Decisión de Producto** — el Blueprint ya define la arquitectura objetivo (Exportar → Perfil y ajustes; Importación → superficie propia). Se registró como deuda de integración en `docs/design/integration-debt.md` (fila 2026-07-22), no como D3. **C1** (colores permanentes rojo/verde en `TransactionPreviewTable.jsx`), **A1** (exportar sin confirmación) y **A2** (importar sin deshacer) resueltos — ver detalle abajo. Con esto, **el alcance de RC-1.4 independiente de D2 queda completo** (C1, A1, A2). **M1–M5 y B1–B6 retirados** por pérdida de trazabilidad documental — ver "Regularización documental" a continuación. **C2 sigue bloqueado únicamente por D2**, sin cambios.

### Regularización documental — retiro de M1–M5, B1–B6 (RC-1.4)

Un relevamiento documental exhaustivo (búsqueda en `docs/release/RC-1.md`, `docs/technical/*`, `docs/design/*`, memoria persistente del proyecto) confirmó que los identificadores **M1, M2, M3, M4, M5, B1, B2, B3, B4, B5, B6** de RC-1.4 no tienen ninguna descripción funcional, archivo afectado, severidad, evidencia ni dependencia recuperable en ningún lugar del proyecto — a diferencia de C1, A1 y A2 del mismo ticket, que sí conservan una descripción de una línea verificable contra el código. A diferencia del caso de RC-1.6 (donde al menos existía registro de que la auditoría completa había sido entregada en una sesión anterior), para estos 11 ítems de RC-1.4 no se encontró siquiera esa constancia — solo aparecen como etiquetas sueltas en la lista de "resto de hallazgos".

**Decisión de ingeniería (PO):**
- El contenido original de estos 11 ítems **no es recuperable**.
- Los identificadores **M1–M5 y B1–B6 de RC-1.4 quedan formalmente retirados**.
- Estos identificadores **no deben reutilizarse** como referencia para trabajo futuro, ni reconstruirse por inferencia a partir del estado actual del código.
- Cualquier mejora futura sobre estas áreas de "Herramientas" más allá de C1/A1/A2 (con trazabilidad completa) deberá originarse en una **auditoría nueva, con numeración propia y documentación persistida desde el inicio** — nunca por referencia a estos identificadores retirados.
...
**A1 — implementado y commiteado (`2cecd69`).** `ExportManager.jsx` ejecutaba la exportación real (CSV/PDF) directamente al hacer click en el botón, sin ningún paso de confirmación previo. Se agrega confirmación reutilizando `ConfirmDialog.jsx` (ya con accesibilidad completa desde RC-1.7/M2 — trampa de foco, Escape) — sin crear un componente nuevo. El gate de plan (`hasFeature('export_csv')`/`hasFeature('export_pdf')`) se evalúa primero, sin cambios: si el usuario no tiene acceso, sigue mostrando `UpgradeModal`; la confirmación aparece únicamente cuando la exportación va a proceder realmente. `handleExportCSV`/`handleExportPDF` se dividieron en un handler de click (gate + abrir confirmación) y una función de ejecución (`executeExportCSV`/`executeExportPDF`, misma lógica de antes sin cambios) disparada solo al confirmar; cancelar no ejecuta nada. Sin tocar `C1` (ya cerrado), `A2`, `ConfirmDialog.jsx` ni ningún otro archivo. Evidencia: 4 tests nuevos (2 preexistentes de exportación se actualizaron para pasar por el paso de confirmación agregado, sin cambiar su aserción original) + 4 tests nuevos dedicados a la confirmación (diálogo se abre en vez de exportar de inmediato; Cancelar no exporta; Confirmar exporta y cierra el diálogo; mismo comportamiento para PDF), suite completa en verde (62 archivos / 705 tests), build en verde, y validación en navegador real (Herramientas → Exportar CSV): el diálogo se abre con el texto "¿Exportar 1 transacciones a CSV?", Cancelar cierra el diálogo sin descargar nada, y al confirmar se dispara la descarga real (`presupuesto_2026-07-01_2026-07-24.csv`) y el diálogo se cierra, sin errores de consola.

**A2 — cerrado documentalmente, sin cambio de código.** El hallazgo original ("importar sin deshacer") describía `ImportManager.jsx` como carente de toda capacidad de Undo. Verificación contra el código actual (no solo el texto de la auditoría, siguiendo el mismo criterio ya aplicado a A4/M3 de RC-1.6): el camino de importación masiva efectivamente usado en producción (`App.jsx` → `handleBulkImportTransaction` → `addBulkTransactions(transactions, { notification: 'toast' })`, siempre disponible como `onBulkImport`) ya dispara el Toast reversible con "Deshacer" implementado en RC-1.7/A4 (`pendingOperation`, kind `'bulkCreate'`) — confirmado con evidencia de navegador real durante el cierre de RC-1.7/A4 (CSV → "1 transacciones importadas" → Deshacer funcional). El hallazgo queda cerrado por resolución efectiva ya existente, sin requerir ninguna implementación nueva. **Cierre documental, sin cambio de código** — por lo tanto sin tests nuevos, sin build nuevo, sin validación en navegador nueva (la evidencia de navegador ya existente de RC-1.7/A4 cubre este comportamiento). Existe un camino de fallback secuencial no utilizado en producción (`onImport` → `handleImportTransaction`, sin `notification:'toast'`, solo alert legacy sin Undo) — anotado como candidato de limpieza futura en la sección "RC-1.8" de este mismo documento (no forma parte del alcance ya cerrado de esa fase, es una anotación para un futuro barrido).
...
Estado:
✅ Hallazgo funcional implementado y commiteado (`ca24cc0`, ver detalle abajo). No genera una nueva Decisión de Producto (no hay D3) ni una nueva entrada en `integration-debt.md`. La auditoría concluyó que la infraestructura de i18n (`react-i18next`, `i18next-browser-languagedetector`, los 3 locales `es`/`en`/`fr` estructuralmente idénticos, `LanguageSelector.jsx` con `i18n.changeLanguage()` funcional) ya existe y funciona correctamente — es decir, la decisión de producto de soportar 3 idiomas ya está tomada y ejecutada en la infraestructura, no falta definirla. Lo que queda pendiente es la adopción/migración gradual de las pantallas activas hacia esa infraestructura (hoy solo `ProfilePage.jsx` la usa de verdad, ~90% de cobertura; el resto de la superficie montada — Dashboard, Historial, Nuevo Movimiento, Herramientas, Omnibar, Gráficos, shell del Design System — está en 0%, con texto hardcodeado en español). Esto se mantiene registrado como **implementación diferida a una fase posterior al Release Candidate**, no como alcance abierto de RC-1 — el roadmap de RC-1.5 se limitaba explícitamente al bug de locale, no a la migración de textos.

**Hallazgo funcional — implementado y commiteado (`ca24cc0`).** `formatCurrency()` (`src/utils/formatters.js`) usaba un locale hardcodeado (`'es-419'`), y `src/features/export/exportUtils.js` usaba un locale hardcodeado distinto (`'es-ES'`) para fechas en CSV/PDF — ninguno de los dos seguía el idioma activo de la aplicación, sin importar qué tan migrada estuviera la traducción de textos. Antes de implementar se definió con el PO el mapeo idioma i18n → locale de formato: `es → es-419` (ya era la convención de facto en el resto de la app — `CurrencyContext.jsx`, `dashboardCalculations.js` — y coherente con el roster de monedas soportadas, centrado en Latinoamérica), `en → en-US` (único precedente sin conflicto, coherente con USD como moneda por defecto), `fr → fr-FR` (sin precedente en conflicto). Se centralizó el mapeo en `getFormatLocale()` (`formatters.js`), que resuelve el locale de formato a partir del idioma activo real de i18next (`i18n.language`, singleton importado directamente) — `formatCurrency()` lo usa internamente, y `exportUtils.js` lo importa y reemplaza sus 6 ocurrencias de `.toLocaleDateString('es-ES')`. Sin tocar `CurrencyContext.jsx`, `dashboardCalculations.js` ni los usos hardcodeados de `en-US` en `SharedSpaceManager.jsx`/`RecurringManager.jsx` (fuera de este alcance por decisión del PO). Efecto secundario: 4 archivos de test que mockeaban `react-i18next` reemplazando el módulo completo (sin reexportar `initReactI18next`) rompieron al importar `formatters.js` (que ahora inicializa el singleton real de i18n) — corregido preservando el módulo real vía `importOriginal` en el mock; `FilaMovimiento.test.jsx` además fija el idioma real a `'es'` (el mock de `useTranslation()` no alcanza, porque `formatCurrency` lee `i18n.language` directamente, no vía ese hook), ya que sus asserts dependen de una particularidad de ICU específica de `es-419` (código de moneda como texto, no símbolo) que el idioma detectado por defecto en jsdom (`en`) no reproduce. Evidencia: 8 tests nuevos en `formatters.test.js` (mapeo de los 3 idiomas + fallback + `formatCurrency` end-to-end en cada idioma) + 9 tests nuevos en `exportUtils.test.js` (archivo nuevo, no existía — CSV y PDF en los 3 idiomas), suite completa en verde (63 archivos / 725 tests), build en verde, y validación en navegador real (Ajustes → selector de idioma, sin recargar la página): con `es`, el Dashboard muestra "20 jul" y "1,234.56 USD"; al cambiar a `en`, "Jul 20" y "1,234.56" (misma agrupación que es-419, difiere de es-419 solo en símbolo/código de moneda); al cambiar a `fr`, "20 juil." y "−1 234,56 $US" (espacio de miles, coma decimal, mes en francés) — confirmado que montos y fechas cambian de formato en vivo al cambiar el idioma, sin recarga.
...
**Candidato para una futura fase de limpieza (no forma parte del alcance ya cerrado arriba, anotado durante el cierre de RC-1.4/A2):** `src/features/import/ImportManager.jsx` tiene un camino de fallback secuencial (`onImport` → `handleImportTransaction` en `App.jsx`) que nunca se ejecuta en producción — `onBulkImport` (el camino preferido, "más rápido") está siempre disponible en el wiring actual de `App.jsx`, así que el fallback es código muerto en la práctica. A diferencia del camino masivo, el fallback usa `showAlert` legacy sin `notification:'toast'`, por lo que tampoco tiene Undo. Candidato a eliminarse (o a recibir Undo, si se decide mantenerlo) en un barrido futuro de deuda técnica.
...
| RC-1.2 | `f8ae378` (C1 — validación de descripción y fin del fallo silencioso en creación), `aff709f` (A2 — orden alfabético de categorías); D1 resuelta; M1 confirmado sin acción; B1–B4 retirados por pérdida de trazabilidad documental | ✅ Alcance implementable completo |
