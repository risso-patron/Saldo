...
Estado:
🟡 Auditoría cerrada — implementación pendiente. Hallazgo C2 (categorías del picker de revisión de importación inconsistentes con `EXPENSE_CATEGORIES`) reclasificado y sacado de alcance — ver "D2" en Decisiones de producto pendientes, arriba (única decisión de producto pendiente de este ticket). Hallazgo A3 (arquitectura de información de "Herramientas" vs. Product Blueprint) analizado y clasificado: **no constituye una Decisión de Producto** — el Blueprint ya define la arquitectura objetivo (Exportar → Perfil y ajustes; Importación → superficie propia). Se registró como deuda de integración en `docs/design/integration-debt.md` (fila 2026-07-22), no como D3. **C1** (colores permanentes rojo/verde en `TransactionPreviewTable.jsx`) y **A1** (exportar sin confirmación) implementados — ver detalle abajo. **A2** (importar sin deshacer — verificado contra el código actual: parcialmente resuelto ya como efecto colateral de RC-1.7/A4) queda como único hallazgo restante independiente de D2, pendiente de aprobación separada. **M1–M5 y B1–B6 retirados** por pérdida de trazabilidad documental — ver "Regularización documental" a continuación.
...
| RC-1.4 | `a34da36` (C1 — colores permanentes en totales de importación), `2cecd69` (A1 — confirmación antes de exportar); D2 pendiente de decisión de producto — bloquea únicamente C2; M1–M5/B1–B6 retirados por pérdida de trazabilidad documental; resto — A2 — sin commit todavía | 🟡 En progreso |
